...
describe GelfLogger::Sender do
  describe 'send' do
    ...
    it 'sends a single packet' do
      expect(socket).to receive(:send).with('test', 'test.example.com', 123)

      sender.send(['test'])
    end

    it 'sends multiple packets' do
      expect(socket).to receive(:send).with('hello', 'test.example.com', 123)
      expect(socket).to receive(:send).with('world', 'test.example.com', 123)
